...
namespace aZaaS.Kstar.DAL
{
    public class CustomExtUtility
    {
        ...
        /// <param name="Status">状态</param>
        /// <returns></returns>
        public static DataSet GetAllProcessInstSign(string processName, string Folio, string StartDate, string FinishDate, string StartUser, int Status, int pageSize, int pageIndex, string SysId)
        {
            SqlParameter[] paras ={new SqlParameter("@ProcessName",processName),
                                 new SqlParameter("@Folio",Folio),
                                  new SqlParameter("@StartTime",StartDate),
                                  ...
                                  new SqlParameter("@pageIndex",pageIndex),
                                 new SqlParameter("@SysId",SysId)};
            var dataSet = SqlHelper.ExecuteDataSet(CommandType.StoredProcedure, "SP_KStar_Common_ProcInstManagerSign", paras);
            return dataSet;
        }
        /// <summary>
        /// 通用流程监控获取管理模块
        ...
        /// <summary>
        /// 获取流程审批人(流程表单上的字段存储的人)
        /// </summary>
        /// <param name="ProcessFullName"></param>
        /// <param name="ActivityName"></param>
        /// <param name="FieldName"></param>
        /// <returns></returns>
        public static string GetProcessApprover(string ProcessFullName, string ActivityName, int FormID)
        {
            List<SqlParameter> paras = new List<SqlParameter>();
            SqlParameter p = new SqlParameter();
            p.SqlDbType = SqlDbType.NVarChar;
            p.Value = ProcessFullName;
            p.Size = 300;
            p.ParameterName = "@ProcessFullName";
            paras.Add(p);

            p = new SqlParameter();
            p.DbType = DbType.String;
            p.Size = 300;
            p.Value = ActivityName;

            p.ParameterName = "@ActivityName";
            paras.Add(p);
            ...
            p.Value = FormID;
            p.Size = 300;
            p.ParameterName = "@FormID";
            paras.Add(p);
            var Approvers = SqlHelper.ExecuteScalar(SqlHelper.Con, CommandType.StoredProcedure, "NW_Common_CustomerRole", paras.ToArray());
            return Approvers == null ? null : Approvers.ToString();
