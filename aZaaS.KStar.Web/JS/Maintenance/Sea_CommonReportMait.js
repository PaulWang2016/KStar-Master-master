...
function Init() {
    ...
    InitServerCustomKendoExcelGridForCommonReport("CommonReport_TableList", model, columns, "/Maintenance/CommonReportMait/Find", {
        ProcessSetID: ProcSetID

    }, 1200, "流程通用报表配置", strUrl,
           function () {
...
var AddReport = function ()
{
    ...
    $("#AddStaffWindow").data("kendoWindow").center().title("通用报表维护").open();
}
var EditReportDisplay = function (id)
{
    if (id != undefined) {
        ...
        ddl.value(model.ProcSetID);
            ...
            $("#AddStaffWindow .windowConfirm").attr("data-url", "/Maintenance/CommonReportMait/EditReportDisplay");
            $("#AddStaffWindow").data("kendoWindow").center().title("通用报表维护").open();
...
var DeleUsers = function () {
    ...
    if (idList.length > 0) {
        bootbox.confirm("你确定删除吗？", function (result) {
            if (result) {
                $.ajax({
                    url: "/Maintenance/CommonReportMait/DeleteDisplay",
                    type: "POST",
                    data: { idList: idList },
                    traditional: true,
...
var AddCommonReportConfig = function () {
    ...
    if (validator.validate()) {
        ...
        {
             url = "/Maintenance/CommonReportMait/AddDisplay";
        }
        //ProcSetID = $("#sltProcess").val();
        var data = {
