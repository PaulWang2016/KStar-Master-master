define(function (require, exports, module) {
    ...
    var worklistconfigcolumns = [
        ...
        {
            field: "ProcessName", title: jsResxMaintenance_SeaWorklist.Process, template: function (item) {
                return "<a href='#widget=/Maintenance/WorklistData-" + item.WorklistID + "'> " + item.ProcessName + "</a>";
            }, filterable: false
        },
        { field: "ApplicationName", title: jsResxMaintenance_SeaWorklist.ApplicationName, filterable: false },
        { field: "ConnectionString", title: jsResxMaintenance_SeaWorklist.ConnectionString, filterable: false },
        { field: "DataTable", title: jsResxMaintenance_SeaWorklist.DataTable, filterable: false },
        { field: "WhereQuery", title: jsResxMaintenance_SeaWorklist.WhereQuery, filterable: false },
    ...
    var AddWConfig = function () {
        ...
        $("#AddWorklistWindow").data("kendoWindow").title(jsResxMaintenance_SeaWorklist.AddWorklistConfig).center().open();
    }
    var EditWConfig = function (id) {
        ...
        if (id) {
            ...
            $("#AddWorklistWindow").data("kendoWindow").title(jsResxMaintenance_SeaWorklist.EditWorklistConfig).center().open();
        }
        else {
            ShowTip(jsResxMaintenance_SeaWorklist.Pleaseselectworklist);
        }
    }
    var DelWConfig = function () {
        ...
        if (idList.length > 0) {
            bootbox.confirm(jsResxMaintenance_SeaWorklist.Areyousure, function (result) {
                if (result) {
                    $.ajax({
    ...
    var CreateAddWorklistWindow = function () {
        ...
        if (!AddWorklistWindow) {
            $("#AddWorklistWindow").kendoWindow({
                width: "850px",
                title: jsResxMaintenance_SeaWorklist.AddWorklistConfig,
                actions: [
                    "Close"
                ],
                open: function (e) {
    ...
    function LoadWorklistView() {
        ...
        $.getJSON("/Maintenance/Worklist/GetWorklist", { _t: new Date() }, function (items) {
            InitKendoExcelGrid("WorklistConfigView", WorklistConfigModel, worklistconfigcolumns, items, 20, jsResxMaintenance_SeaWorklist.WorklistConfiguration, function () {//
        ...
        $.getJSON("/Maintenance/Process/Get", { _t: new Date() }, function (items) {//EmailTpml/GetProcessList
            $("#worklistProcess").kendoDropDownList({
                ...
                optionLabel: jsResxMaintenance_SeaWorklist.SelectProcess
            });
        }).fail(function () {
            $("#worklistProcess").kendoDropDownList({
                dataTextField: "FullName",
        ...
        $("#FetchField").click(FetchConfigField);
    }
    module.exports = LoadWorklistView;
})